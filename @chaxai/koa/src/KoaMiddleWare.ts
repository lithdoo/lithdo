import { type IChaxService, ChaxApiPath, type IChaxStreamChunk } from "@chaxai/core";
 ...
 * - DELETE /ai/record/:conversationId → onDeleteConversation(conversationId)
...
export class ChaxKoaMiddleWare {
    ...
    async handle(ctx: Context, next: Next): Promise<void> {
        ...
        try {
            ...
            if (method === 'DELETE') {
                const deleteMatch = path.match(/^\/ai\/record\/([^/]+)$/);
                if (deleteMatch) {
                    const conversationId = deleteMatch[1];
                    await this.chaxService.onDeleteConversation(conversationId);
                    ctx.status = 204;
                    return;
                }
            }
