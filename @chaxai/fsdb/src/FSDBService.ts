import { IChaxService, IChaxConversation, IChaxMessageInfo, IChaxStreamChunk, IChaxConversationManagerBuilder } from "@chaxai/core";
...
export abstract class ChaxFSDBService implements IChaxService {
    ...
    static build(conversationManagerBuilder: IChaxConversationManagerBuilder, dirPath: string = process.env.CHAXAI_FILE_DIR || path.join(process.cwd(), './.chaxai_data')) {
        return new class  extends ChaxFSDBService {
    ...
    constructor(
        public readonly dirPath: string = process.env.CHAXAI_FILE_DIR || path.join(process.cwd(), './[FSDB]chaxai_data'),
    ) {

        if(!this.getConversationListPath()) {
            fs.mkdirSync(this.getConversationListPath(), { recursive: true });
        }
        if(!this.getContentRootPath()) {
            fs.mkdirSync(this.getContentRootPath(), { recursive: true });
        }
        if(!this.getConversationMessagesPath()) {
            fs.mkdirSync(this.getConversationMessagesPath(), { recursive: true });
        }
    }
     ...
     * @returns 会话目录的绝对路径
     */
    getConversationListPath() {
        return path.join(this.dirPath, './[struct]conversation');
    }
     ...
     */
    getContentRootPath() {
        return path.join(this.dirPath, './[resource]content');
    }
    ...
    getConversationMessagesPath() {
        return path.join(this.dirPath, './[extend]message');
    }
    ...
    //     return path.join(this.getMessageRootPath(), 'list.json');
    // }
    ...
    getConversationMessageListJsonPath(conversationId: string) {
        return path.join(this.getConversationMessagesPath(), `${conversationId}.json`);
    }
    ...
    getMessageContentPath(msgId: string) {
        return path.join(this.getContentRootPath(), `${msgId}.txt`);
    }
    ...
    getMessageErrorPath(msgId: string) {
        return path.join(this.getContentRootPath(), `${msgId}.error.txt`);
    }
    ...
    async onFetchChatConversations(): Promise<IChaxConversation[]> {
        const listPath = this.getConversationListPath();
        try {
            const list = fs.readdirSync(listPath)
                .filter(f => f.endsWith('.json'))
                .map(v => fs.readFileSync(path.join(listPath, v), 'utf-8'))
                .map(v => JSON.parse(v) as IChaxConversation)
                .filter(v => !v.isDeleted)
                .sort((a, b) => {
                    // 根据 updateTimestamp 排序，越大的排前面
    ...
    getConversation(conversationId: string) {
        ...
        const listPath = this.getConversationListPath();
        const filePath = path.join(listPath, `${conversationId}.json`);
        try {
            // 确保目录存在
            try {
                fs.accessSync(this.getConversationListPath());
            } catch {
                fs.mkdirSync(this.getConversationListPath(), { recursive: true });
            }
            ...
            return JSON.parse(fs.readFileSync(filePath, 'utf-8')) as IChaxConversation
        } catch (error) {
            throw error;
        }
    }
    ...
    updateConversationMessageList(conversation: IChaxConversation, message: IChaxMessageInfo, data: {
        ...
        try {
            fs.accessSync(this.getConversationMessagesPath());
        } catch {
            fs.mkdirSync(this.getConversationMessagesPath(), { recursive: true });
        }
        ...
        // 确保消息存储目录存在
        try {
            fs.accessSync(this.getContentRootPath());
        } catch {
            fs.mkdirSync(this.getContentRootPath(), { recursive: true });
        }
        try {
    ...
    async updateConversationInfo(info:IChaxConversation): Promise<void> {
       const filePath = path.join(this.getConversationListPath(), `${info.conversationId}.json`);
